...
The [BiblioFuse Web Tool](/en/webapp/) handles the renumbering automatically. You supply the files and specify the order; the tool extracts the images from each CBZ, sequences them correctly, and packages everything into a single output CBZ.

## How to Merge CBZ Files Online

**Step 1: Open the BiblioFuse Web Tool**

Open [BiblioFuse Web Tool](/en/webapp/) in a desktop browser. No account or login required. The merge feature is available in the free tier for batches of up to three files; upgrade for larger batches.
...
The [BiblioFuse Web Tool](/en/webapp/) can merge more than just CBZ files. If your series uses a mix of formats — some chapters as CBZ, some as CBR, some as ZIP — the tool handles them together. You don't need to convert everything to the same format before merging. The tool extracts pages from each input format and packages the result as CBZ.
...
If you want to reduce file size at the same time as merging, [BiblioFuse Web Tool](/en/webapp/) also has a compression step you can run before or after the merge — re-encoding images at a chosen quality level to shrink the final file. But that's optional; a straight merge preserves everything exactly.
...
2. Open [BiblioFuse Web Tool](/en/webapp/) and add those chapter files in order.
...
Open [BiblioFuse](/en/) on your iPhone, tap the Wi-Fi Transfer button, and a browser address appears on screen. On your computer, open that address in a browser. Drag your merged CBZ file onto the page. It transfers immediately over your local network — no USB, no iTunes, no cloud account needed.
...
CBR is a RAR archive, not a ZIP. The [BiblioFuse Web Tool](/en/webapp/) handles CBR alongside CBZ and other formats — you can add a mix of CBZ and CBR files and they'll merge correctly into a single CBZ output.
...
The [BiblioFuse Web Tool](/en/webapp/) handles CBZ merging the way it should work — in the browser, privately, for free, without making you install anything. You supply the files in order and it handles the rest.
...
Open [BiblioFuse Web Tool](/en/webapp/) and merge your first batch.
