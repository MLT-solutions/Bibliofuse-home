...
1. **Téléchargez et ouvrez Comic Duplicate Scanner** depuis le Mac App Store ou le site web du développeur.
...
Comic Duplicate Scanner gère tous les formats qu'un photographe Mac peut rencontrer :
...
Après chaque migration de Mac, l'Assistant Migration d'Apple copie les fichiers mais ne supprime pas les doublons. Si vous avez migré plusieurs fois entre des Macs, vous avez probablement deux ou trois copies de vos photos les plus anciennes. Exécutez Comic Duplicate Scanner sur votre dossier personnel pour les retrouver toutes.

**2. Paires RAW + JPEG**
De nombreux appareils photo — ainsi que le mode ProRAW de l'iPhone — enregistrent simultanément un fichier RAW et un JPEG. Après retouche dans Lightroom ou Capture One, vous n'avez souvent plus besoin du JPEG d'origine. Comic Duplicate Scanner regroupe les fichiers RAW avec leurs équivalents JPEG pour vous permettre de supprimer les fichiers inutiles sans trier manuellement par date.

**4. Photo couleur versus export en noir et blanc**
Vous avez pris une photo en couleur mais exporté une version en noir et blanc pour un projet, puis oublié laquelle est laquelle. Comme le hachage perceptuel compare la structure de luminance plutôt que les valeurs de couleur, un original en couleur et son export en niveaux de gris produisent des hashes presque identiques et sont regroupés comme doublons — même si un outil basé sur les octets les voit comme des fichiers complètement différents.
...
### Comic Duplicate Scanner supprime-t-il les photos définitivement ?
...
### Peut-il faire correspondre une photo couleur avec une version en noir et blanc de la même image ?

Oui. L'algorithme dHash convertit chaque image en niveaux de gris avant de calculer son empreinte, ce qui signifie qu'il compare des modèles de luminance plutôt que des valeurs de couleur. Une photo en couleur et son export en noir et blanc produisent des hashes presque identiques et sont regroupés comme doublons.

### La résolution de l'image affecte-t-elle la détection des doublons ?

Non. dHash redimensionne chaque image vers une petite grille fixe avant de comparer, donc un téléchargement web basse résolution et un original haute résolution de la même photo produisent le même hash. Les différences de résolution entre les sources sont complètement ignorées.

### Comic Duplicate Scanner est-il disponible pour Windows ?

Oui. Comic Duplicate Scanner est disponible pour Mac et Windows. La version Windows dispose du même moteur de hachage perceptuel et prend en charge les mêmes formats de fichiers.

## Récupérez votre espace de stockage — sans risque

Une bibliothèque de 40 000 photos avec 30 % de doublons gaspille des dizaines de gigaoctets d'espace disque. Comic Duplicate Scanner vous donne les outils pour la nettoyer en toute sécurité — hachage perceptuel qui détecte les doublons visuels entre formats, stratégies de sélection automatique qui éliminent les approximations, et suppression vers la Corbeille uniquement qui vous protège.

[Téléchargez Comic Duplicate Scanner](/fr/archive/) et lancez votre première analyse dès aujourd'hui. Si vous souhaitez également compresser ou convertir par lots vos collections de photos, l'[outil web BiblioFuse](/fr/webapp/) gère les fichiers EPUB, CBZ et PDF entièrement dans votre navigateur.
