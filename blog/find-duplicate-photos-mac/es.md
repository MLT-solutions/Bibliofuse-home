...
1. **Descarga y abre Comic Duplicate Scanner** desde la Mac App Store o el sitio web del desarrollador.
...
5. **Elige tu estrategia de eliminación** — Comic Duplicate Scanner ofrece estrategias de selección automática: conservar el archivo más grande (mayor calidad), conservar el más antiguo (el original) o conservar el que está en una carpeta preferida. También puedes seleccionar manualmente.
...
Comic Duplicate Scanner gestiona todos los formatos que encuentra un fotógrafo en Mac:
...
Después de cada migración de Mac, el Asistente de Migración de Apple copia los archivos pero no elimina los duplicados. Si has migrado entre varios Macs, probablemente tienes dos o tres copias de tus fotos más antiguas. Ejecuta Comic Duplicate Scanner en tu carpeta de inicio para encontrarlas todas.

**2. Pares RAW + JPEG**
Muchas cámaras —y el modo ProRAW del iPhone— guardan simultáneamente un archivo RAW y un JPEG. Después de editar en Lightroom o Capture One, a menudo ya no necesitas el JPEG original. Comic Duplicate Scanner agrupa los archivos RAW con sus contrapartes JPEG para que puedas limpiar los extras sin ordenar manualmente por fecha.

**4. Foto en color frente a exportación en blanco y negro**
Fotografiaste en color pero exportaste una versión en blanco y negro para un proyecto y olvidaste cuál era cuál. Como el hash perceptual compara la estructura de luminancia en lugar de los valores de color, un original en color y su exportación en escala de grises producen hashes casi idénticos y se agrupan como duplicados, aunque una herramienta de bytes los vea como archivos completamente diferentes.
...
### ¿Comic Duplicate Scanner elimina las fotos de forma permanente?
...
### ¿Puede comparar una foto en color con una versión en blanco y negro de la misma imagen?

Sí. El algoritmo dHash convierte cada imagen a escala de grises antes de calcular su huella digital, lo que significa que compara patrones de luminancia en lugar de valores de color. Una foto a todo color y su exportación en blanco y negro producen hashes casi idénticos y se agrupan como duplicados.

### ¿Afecta la resolución de la imagen a la detección de duplicados?

No. dHash redimensiona cada imagen a una cuadrícula fija pequeña antes de comparar, por lo que una descarga web de baja resolución y un original de alta resolución de la misma foto producen el mismo hash. Las diferencias de resolución entre fuentes se ignoran por completo.

### ¿Está disponible Comic Duplicate Scanner para Windows?

Sí. Comic Duplicate Scanner está disponible tanto para Mac como para Windows. La versión de Windows tiene el mismo motor de hash perceptual y es compatible con los mismos formatos de archivo.

## Recupera tu almacenamiento sin riesgos

Una biblioteca de 40.000 fotos con un 30 % de duplicados desperdicia decenas de gigabytes de almacenamiento. Comic Duplicate Scanner te ofrece las herramientas para limpiarla de forma segura: hash perceptual que detecta duplicados visuales entre formatos, estrategias de selección automática que eliminan las conjeturas y eliminación solo a la Papelera que te mantiene protegido.

[Descarga Comic Duplicate Scanner](/es/archive/) y comienza tu primer análisis hoy. Si también quieres comprimir o convertir por lotes tus colecciones de fotos, la [herramienta web de BiblioFuse](/es/webapp/) gestiona archivos EPUB, CBZ y PDF completamente en tu navegador.
