...
[Comic Duplicate Scanner](/pt/archive/) encontra fotos duplicadas no Mac usando hashing perceptual — uma técnica que identifica imagens visualmente idênticas mesmo que tenham nomes de ficheiro, formatos ou configurações de compressão diferentes. Suporta HEIC, WebP, JPEG, PNG, TIFF, RAW e dezenas de outros formatos. A aplicação está disponível para Mac e Windows e move os duplicados para o Lixo com segurança, em vez de os eliminar permanentemente.
...
1. **Descarregue e abra o Comic Duplicate Scanner** a partir da Mac App Store ou do site do programador.
...
5. **Escolha a sua estratégia de eliminação** — o Comic Duplicate Scanner oferece estratégias de seleção automática: manter o ficheiro maior (maior qualidade), manter o mais antigo (original) ou manter o que está numa pasta preferida. Também pode selecionar manualmente.
...
Comic Duplicate Scanner suporta todos os formatos que um fotógrafo Mac encontra:
...
Após cada migração de Mac, o Assistente de Migração da Apple copia ficheiros mas não os deduplica. Se já mudou de Mac mais de uma vez, provavelmente tem duas ou três cópias das suas fotos mais antigas. Execute o Comic Duplicate Scanner na sua pasta pessoal para as encontrar todas.

**2. Pares RAW + JPEG**
Muitas câmaras — e o modo ProRAW do iPhone — guardam simultaneamente um RAW e um JPEG. Após editar no Lightroom ou Capture One, muitas vezes já não precisa do JPEG original. O Comic Duplicate Scanner agrupa ficheiros RAW com os seus equivalentes JPEG para que possa limpar os extras sem ordenar manualmente por data.

**4. Foto colorida versus exportação em preto e branco**
Você fotografou em cores mas exportou uma versão em preto e branco para um projeto e esqueceu qual era qual. Como o hash perceptual compara a estrutura de luminância em vez dos valores de cor, um original colorido e sua exportação em escala de cinza produzem hashes quase idênticos e são agrupados como duplicatas — mesmo que uma ferramenta de bytes os veja como arquivos completamente diferentes.
...
### O Comic Duplicate Scanner elimina fotos permanentemente?
...
### Ele pode comparar uma foto colorida com uma versão em preto e branco da mesma imagem?

Sim. O algoritmo dHash converte cada imagem para escala de cinza antes de calcular sua impressão digital, o que significa que compara padrões de luminância em vez de valores de cor. Uma foto colorida e sua exportação em preto e branco produzem hashes quase idênticos e são agrupados como duplicatas.

### A resolução da imagem afeta a detecção de duplicatas?

Não. O dHash redimensiona cada imagem para uma grade fixa pequena antes de comparar, então um download web de baixa resolução e um original de alta resolução da mesma foto produzem o mesmo hash. As diferenças de resolução entre fontes são completamente ignoradas.

### O Comic Duplicate Scanner está disponível para Windows?

Sim. O Comic Duplicate Scanner está disponível para Mac e Windows. A versão Windows tem o mesmo motor de hashing perceptual e suporta os mesmos formatos de ficheiro.

## Recupere o Seu Espaço de Armazenamento — Sem Riscos

Uma biblioteca de 40 000 fotos com 30% de duplicados desperdiça dezenas de gigabytes de armazenamento. O Comic Duplicate Scanner dá-lhe as ferramentas para a limpar com segurança — hashing perceptual que deteta duplicados visuais entre formatos, estratégias de seleção automática que eliminam as suposições, e eliminação apenas para o Lixo que o mantém protegido.

[Descarregue o Comic Duplicate Scanner](/pt/) e inicie a sua primeira análise hoje. Se também quiser comprimir ou converter em lote as suas coleções de fotos, a [Ferramenta Web BiblioFuse](/pt/webapp/) suporta ficheiros EPUB, CBZ e PDF inteiramente no seu browser.
