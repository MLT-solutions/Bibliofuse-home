...
Perceptual hashing looks at what an image *looks like*, not what its bytes say. Comic Duplicate Scanner computes a compact visual fingerprint for every photo. Two images that look identical to the human eye produce the same (or very similar) fingerprint — even if one is HEIC and the other is JPEG, even if one was re-exported at 85% quality, and even if someone renamed the file.
...
1. **Download and open Comic Duplicate Scanner** from the Mac App Store or the developer website.
...
5. **Choose your deletion strategy** — Comic Duplicate Scanner offers auto-select strategies: keep the largest file (highest quality), keep the oldest (original), or keep the one in a preferred folder. You can also select manually.
...
Comic Duplicate Scanner handles every format a Mac photographer encounters:
...
After every Mac migration, Apple's Migration Assistant copies files but doesn't deduplicate. If you've moved between Macs more than once, you likely have two or three copies of your oldest photos. Run Comic Duplicate Scanner on your home folder to find them all.

**2. RAW + JPEG pairs**
Many cameras — and the iPhone's ProRAW mode — save a RAW and a JPEG simultaneously. After editing in Lightroom or Capture One, you often no longer need the original JPEG. Comic Duplicate Scanner groups RAW files with their JPEG counterparts so you can clean up the extras without manually sorting by date.
...
**4. Color photo vs black-and-white export**
You shot in colour but exported a black-and-white version for a project, then forgot which is which. Because perceptual hashing compares luminance structure rather than colour values, a colour original and its greyscale export produce nearly identical hashes and are grouped as duplicates — even though a byte-level tool sees them as completely different files.

## Frequently Asked Questions

### Does Comic Duplicate Scanner permanently delete photos?
...
### Can it match a colour photo with a black-and-white version of the same image?

Yes. The dHash algorithm converts every image to greyscale before computing its fingerprint, which means it compares luminance patterns rather than colour values. A full-colour photo and a black-and-white export of the same shot produce nearly identical hashes and will be grouped as duplicates.

### Does image resolution affect duplicate detection?

No. dHash resizes every image to a small fixed grid before comparing, so a low-resolution web download and a high-resolution original of the same photo produce the same hash. Resolution differences between sources are ignored entirely.

### Is Comic Duplicate Scanner available for Windows?

Yes. Comic Duplicate Scanner is available for both Mac and Windows. The Windows version has the same perceptual hashing engine and supports the same file formats.

## Recover Your Storage — Without the Risk

A 40,000-photo library with 30% duplicates wastes tens of gigabytes of storage. Comic Duplicate Scanner gives you the tools to clean it up safely — perceptual hashing that catches visual duplicates across formats, auto-select strategies that take the guesswork out, and Trash-only deletion that keeps you protected.

[Download Comic Duplicate Scanner](/en/archive/) and start your first scan today. If you also want to compress or batch-convert your photo collections, the [BiblioFuse Web Tool](/en/webapp/) handles EPUB, CBZ, and PDF files entirely in your browser.
